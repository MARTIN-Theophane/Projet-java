...
public class Joueur extends JFrame {
	...
	public Joueur(ArrayList<Piece> pieces,Lumière lumiere_gauche,Lumière lumiere_droite) {
		...
		System.out.println("Ouvrir/fermer les caméras : Espace");
		System.out.println("Changer de caméras : Flèche droite/gauche");
		System.out.println("Afficher la liste des touches : ALT + F1");
		...
		switch (e.getKeyCode()) {
            case KeyEvent.VK_RIGHT:
    			if (cam.getEtat()) {
    				...
    				if (cam.getNum_piece() == 1 || cam.getNum_piece() == 2) {
    					num_piece += 1;
    		...
    		case KeyEvent.VK_LEFT:
    			if (cam.getEtat()) {
    				int num_piece = cam.getNum_piece();
    				if (cam.getNum_piece() == 6 || cam.getNum_piece() == 5) {
    					num_piece -= 1;
    					cam.setNum_piece(num_piece);
    					Window[] fenetres = Window.getWindows();
    					cam.fermerCameras();
